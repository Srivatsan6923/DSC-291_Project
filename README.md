...
## Paper reproduction (full tables & figures)

Step-by-step run order, commands, and expected outputs for every `script*.sh` in the repo root are documented in **[EXPERIMENTS.md](./EXPERIMENTS.md)**.

Quick start:

```bash
bash script0_confirm_fix.sh    # sanity check
bash script1.sh                # Table 2 / S4 — AG-News training
# … see EXPERIMENTS.md for full order
```
